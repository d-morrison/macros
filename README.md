...
>
> **Note on `\v` in PDF via `include-in-header`:** In LaTeX/PDF output, font-encoding packages (such as `fontspec` or `puenc.def`) re-assert LaTeX's built-in text caron/háček accent `\v` at `\begin{document}`, overriding the preamble's `\renewcommand{\v}[1]{\vecf{#1}}`. For vector notation in PDF documents using `include-in-header`, use `\vec{...}` or `\vecf{...}` (which work across all formats and engines). If your document requires the `\v` shorthand specifically, add `\AtBeginDocument{\renewcommand{\v}[1]{\vecf{#1}}}` to your LaTeX preamble. Documents using the `{{< include >}}` shortcode recipe are unaffected because pandoc's macro expansion pass expands `\v` into `\tilde` during markdown processing before LaTeX compiles.
