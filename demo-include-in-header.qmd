...
- **PDF**: `macros.qmd` goes into the LaTeX preamble via `include-in-header` — macros work, including `\providecommand` (note that LaTeX's font-encoding packages re-assert `\v` as a text caron accent at `\begin{document}`, so `\v{x}` produces no vector notation under `include-in-header` unless re-asserted in the preamble; use `\vec{x}` or `\vecf{x}` for robust cross-format vector notation).
