...
> **Note (HTML/RevealJS):** `macros-header.html` is a MathJax configuration script (not raw `$$...$$` text). It defines macros through `MathJax.tex.macros`, so macros are available before page/slide math is typeset. [View the RevealJS slides output](demo-include-in-header-slides.html).
...
- **HTML**: `macros-header.html` is inserted via `include-in-header` as a MathJax config script. Macro names are set in `MathJax.tex.macros` before typesetting.
- **RevealJS**: `macros-header.html` is inserted via `include-in-header` as the same MathJax config script, so slide math can use the macros globally.
