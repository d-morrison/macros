...
format:
  ...
  revealjs:
    output-file: demo-include-in-header-slides.html
    include-in-header:
      - macros-header.html   # MathJax config script
  docx: default
```

> **Note (HTML/RevealJS):** `macros-header.html` is a MathJax configuration script (not raw `$$...$$` text). It defines macros for both MathJax v3 (`MathJax.tex.macros`) and MathJax v2 (`MathJax.TeX.Macros`), so macros are available before page/slide math is typeset. [View the RevealJS slides output](demo-include-in-header-slides.html).
...
- **HTML**: `macros-header.html` is inserted via `include-in-header` as a MathJax config script. Macro names are set before typesetting.
- **RevealJS**: `macros-header.html` is inserted via `include-in-header` as the same config script (including MathJax v2 support), so slide math can use the macros globally.
