...
> **Why `include-before-body` for RevealJS?** RevealJS processes MathJax per-slide. Macro definitions must be in the `<body>` — before the RevealJS presentation container — to be registered globally before any slide math renders. `include-before-body` places the hidden div at the start of `<body>`, whereas `include-in-header` inserts into `<head>` where RevealJS's MathJax may not pick up definitions reliably.
