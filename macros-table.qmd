...
macros$interpretation[is.na(macros$interpretation)] <- ""

# Restore the original row order (merge() sorts by the key column)
macros <- macros[order(match(macros$name, parse_macros("macros.qmd")$name)), ]
...
DT::datatable(
  macros[, c("name", "args", "interpretation", "definition", "example_source", "example_rendered")],
  colnames  = c("Macro", "# Args", "Interpretation", "Definition", "Example Source", "Example"),
  rownames  = FALSE,
  escape    = FALSE,
  filter    = "top",
  options   = list(
