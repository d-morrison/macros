...
# Embed macro definitions in a hidden display-math block so MathJax defines
# them globally for the rendered example column below.
...
DT::datatable(
  ...
  options   = list(
    ...
    drawCallback = DT::JS(
      "function() { if (window.MathJax) { MathJax.typesetPromise(); } }"
    )
  )
)
```
