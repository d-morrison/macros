...
% ── Estimator macros (hat-wrapped estimand equivalents) ───────────────────────────
% AI NOTE: For every new estimand macro group added above, add corresponding
% estimator (hat-wrapped) versions in this section, following these rules:
%   1. Zero-arg:  \def\hfoo{\hat{\foo}}
%   2. One-arg function:  \providecommand{\hfoof}[1]{\hfoo\paren{#1}}
%   3. One alias per estimand alias:  \def\hfooalias{\hfoo}  (or \hfoofaliasf{\hfoof})
%   4. Add entries for ALL new macros to interpretations.tsv
% See issue #58 for background and motivation.
% Estimated probability / risk / prevalence (all = hat-pi)
\def\hprob{\hat{\prob}}
\def\hrisk{\hat{\risk}}
\def\hprev{\hat{\prev}}
% Estimated odds (hat-omega)
\def\hodds{\hat{\odds}}
% Estimated rate and hazard (hat-lambda; note: \hl = \hat\lambda is a generic alias)
\def\hrate{\hat{\rate}}
\def\hhaz{\hat{\haz}}
% Estimated cumulative hazard (hat-Lambda)
\def\hcuhaz{\hat{\cuhaz}}
\def\hcumhaz{\hcuhaz}
% Generic estimated ratio (hat-theta)
\def\hratio{\hat{\ratio}}
\providecommand{\hratiof}[1]{\hratio\paren{#1}}
% Estimated odds ratio (hat-theta_omega)
\def\hror{\hat{\ror}}
\providecommand{\horf}[1]{\hror\paren{#1}}
% Estimated hazard ratio (hat-theta_lambda)
...
\def\hrhazf{\hhrf}
\def\hhazrf{\hhrf}
% Estimated rate ratio (hat-theta_lambda)
...
% Estimated risk ratio (hat-theta_pi)
...
% Estimated cumulative hazard ratio (hat-theta_Lambda)
...
% Generic estimated factor (hat-theta)
\def\hfactor{\hat{\factor}}
\providecommand{\hfactorf}[1]{\hfactor\paren{#1}}
\def\hff{\hfactorf}
% Estimated odds factor (hat-theta_omega)
...
% Estimated hazard factor (hat-theta_lambda)
...
% Estimated rate factor (hat-theta_lambda)
...
% Estimated risk factor (hat-theta_pi)
...
% Estimated prevalence factor (hat-theta_pi)
...
% Estimated cumulative hazard factor (hat-theta_Lambda)
...
% Estimated survival factor (hat-theta_S)
